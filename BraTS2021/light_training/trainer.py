...
class Trainer:
    ...
    def get_dataloader(self, dataset, shuffle=False, batch_size=1, train=True, num_workers=8):
        if dataset is None :
            return None
        if self.env_type == 'pytorch':
    ...
    def train(self,
        ...
        train_loader = self.get_dataloader(train_dataset, shuffle=True, batch_size=self.batch_size, num_workers=self.num_workers)
        if val_dataset is not None:
            val_loader = self.get_dataloader(val_dataset, shuffle=False, batch_size=1, train=False, num_workers=self.num_workers)
        else :
            val_loader = None 

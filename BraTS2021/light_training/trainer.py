...
class Trainer:
    def __init__(
        ...
        assert env_type in ["pytorch", "ddp", "DDP"], (
            f"not support this env_type: {env_type}"
        )
    ...
    def get_dataloader(
        self, dataset, shuffle=False, batch_size=1, train=True, num_workers=8
    ):
        if dataset is None:
            return None
        if self.env_type == "pytorch":
    ...
    def get_dist_args(self):
        ...
        parser.add_argument(
            "--not_call_launch", action="store_true", help="not call launch!"
        )
        ...
        for epoch in range(0, self.max_epochs):
            ...
            if (epoch + 1) % self.val_every == 0 and val_loader is not None:
                ...
                if self.ddp:
                    ...
                    val_outputs = distributed_concat(
                        val_outputs, num_total_examples=len(val_loader.sampler.dataset)
                    )
                else:
                    val_outputs = torch.tensor(val_outputs)
        ...
        if self.local_rank == 0:
            ...
            for idx, batch in enumerate(loader):
                ...
                if isinstance(batch, dict):
                    ...
                    batch = [
                        x.to(self.device) for x in batch if isinstance(x, torch.Tensor)
                    ]
